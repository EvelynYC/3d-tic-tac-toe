...
export class AnimationManager {
  private static readonly tokenConfig: TokenAnimationConfig = {
    duration: 1.0,
    startHeight: 3,
    swayAmplitude: 0.05,
    swayFrequency: 8,
  ...
  private static readonly victoryConfig: VictoryAnimationConfig = {
    rotationSpeed: 0.5,
    floatAmplitude: 0.3,
    floatFrequency: 2,
    scaleAmplitude: 0.1,
  ...
  static createVictoryAnimation(
    ...
    const finalConfig = { ...this.victoryConfig, ...config };
    const {
      rotationSpeed,
      floatAmplitude,
      floatFrequency,
    ...
    return {
      ...
      particles: this.generateParticles(time, particleCount),
    };
  }
  ...
  static generateTokenRotations(): { [key: string]: [number, number, number] } {
